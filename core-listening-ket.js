...
class ListeningCore {
    ...
    async isCompleted() {
        ...
        if (window.CloudStorage) {
            ...
            // Chỉ coi là hoàn thành nếu status là 'completed' hoặc có điểm số (legacy)
    ...
    saveDraft() {
        ...
        this.debounceTimer = setTimeout(() => {
            try {
                if (this.isLoadingDraft) return; // Đang load thì không lưu
                const draft = this.getDraftData();
                if (!this.draftHasAnswers(draft)) return; // Không lưu nháp rỗng (tránh ghi đè)
    ...
    saveDraftImmediate() {
        ...
        try {
            const draft = this.getDraftData();
            if (!this.draftHasAnswers(draft)) return; // Không lưu nháp rỗng
    ...
    draftHasAnswers(draft) {
        ...
        // Loại bỏ metadata để kiểm tra xem thực sự có đáp án không
        ...
        if ((!draft || !this.draftHasAnswers(draft)) && window.CloudStorage) {
            ...
            try {
                ...
                if (cloudData && this.draftHasAnswers(cloudData)) {
                    ...
                    // CloudStorage.load trả về object, đảm bảo draft là object
                    draft = (typeof cloudData === 'string') ? JSON.parse(cloudData) : cloudData;
                    
                    // Chỉ lưu local nếu không phải cloud_only
                    if (!isCloudOnly) {
    ...
    createNavigation() {
        ...
        prevPartBtn.title = 'Previous Part';
    ...
    restoreSubmittedState(submittedState) {
        ...
        } else if (this.currentTestData.type === 'fill-blank' || this.currentTestData.type === 'match-pairs') {
            for (let i = questionRange.start; i <= questionRange.end; i++) {
                const input = document.getElementById(`q${i}`);
...
class UIManager {
    ...
    _showCriticalStorageError() {
        ...
        overlay.innerHTML = `
            <div style="background: #fef2f2; padding: 28px; border-radius: 16px; max-width: 400px; width: 90%; text-align: center; box-shadow: 0 20px 60px rgba(0,0,0,0.4); border: 2px solid #ef4444;">
                ...
                <p style="margin: 0 0 20px; font-size: 14px; color: #4b5563; line-height: 1.5;">Bộ nhớ trình duyệt đã <strong>hoàn toàn đầy</strong>.</p>
                <div style="display: flex; flex-direction: column; gap: 10px;">
    ...
    async _showCloudOnlyManageModal() {
        ...
        if (!isOnline) {
            ...
            const canReturnHybrid = total < 1 * 1024 * 1024;
